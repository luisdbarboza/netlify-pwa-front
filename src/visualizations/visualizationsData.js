...
const visualizations = {
  ...
  selection: {
    ...
    transition: (svg, mode, data, special) => {
      if (
        ...
        low =
          nodesSVG['rectArray'][special.previousLow] &&
          Number(nodesSVG['rectArray'][special.previousLow].getAttribute('y'));

        target =
          nodesSVG['rectArray'][special.previousTarget] &&
          Number(
  ...
  linkedList: {
    algorithm: (dataStructure, stepData, externalInputs) => {
      const linkedList = dataStructure;
      const markers = [];
      ...
      return {
        transformedDataStructure: linkedList,
        special: {
          ...stepData,
          action: externalInputs.action,
          markers,
    ...
    keyframe: (svg, data, special) => {
      ...
      while (currentNode) {
        ...
        svg
          ...
          .attr('d', () => {
            return counter === 1
              ? `M200 ${nodeY + 20} v50 h-5 l5 5 l5 -5 h-5`
              : `M200 ${nodeY + 20} v50 h-5 l5 5 l5 -5 h-5`;
          })
          .attr('stroke', 'black')
          .attr('fill', 'black')
