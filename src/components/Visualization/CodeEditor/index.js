...
const CodeEditor = ({ fetchedData, selectedTab, selectedVisualization }) => {
  ...
  useEffect(() => {
    if (!fetchedData.loading) {
      ...
      if (!fetchedData.multiple) {
        setCodePath(visualizationsData[visualizationData.name].code);
      } else {
        setCodePath(
          visualizationsData[visualizationData[selectedVisualization].name].code
        );
      }
    }
  }, [fetchedData, selectedVisualization]);
